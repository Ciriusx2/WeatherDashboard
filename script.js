const apikey = `501134d05c8b9943c1e90143ee5a29a2`;
...
function weatherReport(data) {
  var urlcast =
    `http://api.openweathermap.org/data/2.5/forecast?q=${data.name}&appid=${apikey}`;
...
function hourForecast(forecast) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    console.log(
      date.toLocaleTimeString("en-US", { timeZone: "Asia/Kolkata" }).replace(":00", "")
    );
    ...
    temp.innerText =
      Math.floor(forecast.list[i].main.temp_max - 273.15) +
      " °C" +
      " / " +
      Math.floor(forecast.list[i].main.temp_min - 273.15) +
      " °C";
